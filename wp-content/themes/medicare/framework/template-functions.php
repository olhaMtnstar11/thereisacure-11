...
if ( ! function_exists( 'ro_theme_setup' ) ) {
	function ro_theme_setup() {
		...
		register_nav_menus( array(
			'main_navigation'   => __( 'Main Navigation','medicare' ),
		) );
		...
		add_image_size( 'medicare-slider-blog-thumbnail', 420, 280, true );
		add_image_size( 'medicare-slider-doctor-small-thumbnail', 160, 160, true );
...
if (!function_exists('ro_theme_favicon')) {
	function ro_theme_favicon() {
		global $tb_options;
		$icon = isset($tb_options['tb_favicon_image']['url']) && $tb_options['tb_favicon_image']['url'] ? $tb_options['tb_favicon_image']['url']: URI_PATH.'/favicon.ico';
...
if (!function_exists('ro_theme_logo')) {
	function ro_theme_logo() {
		global $tb_options;
		$logo = isset($tb_options['tb_logo_image']['url']) ? $tb_options['tb_logo_image']['url'] : URI_PATH.'/assets/images/medicare_logo.png';
		echo '<img src="'.esc_url($logo).'" alt="'.__('Logo', 'medicare').'">';
	}
}
...
if (!function_exists('ro_theme_wp_title')) {
    function ro_theme_wp_title( $title, $sep ) {
            ...
            if ( $paged >= 2 || $page >= 2 ) {
                    $title = "$title $sep " . sprintf( __( 'Page %s', 'medicare' ), max( $paged, $page ) );
            }
            return $title;
    }
}
...
if (!function_exists('ro_theme_page_title')) {
    function ro_theme_page_title() { 
			...
			if( is_home() ){
                ...
                _e('Search Keyword: ', 'medicare'). '<span class="keywork">'. get_search_query( false ). '</span>';
            }elseif (!is_archive()) {
                the_title();
            } else { 
                if (is_category()){
                    ...
                    printf(__('Author: %s', 'medicare'), '<span class="vcard">' . get_the_author() . '</span>');
                }elseif (is_day()){
                    printf(__('Day: %s', 'medicare'), '<span>' . get_the_date() . '</span>');
                }elseif (is_month()){
                    printf(__('Month: %s', 'medicare'), '<span>' . get_the_date() . '</span>');
                }elseif (is_year()){
                    printf(__('Year: %s', 'medicare'), '<span>' . get_the_date() . '</span>');
                }elseif (is_tax('post_format', 'post-format-aside')){
                    _e('Asides', 'medicare');
                }elseif (is_tax('post_format', 'post-format-gallery')){
                    _e('Galleries', 'medicare');
                    ...
                    _e('Statuses', 'medicare');
                }elseif (is_tax('post_format', 'post-format-audio')){
                    _e('Audios', 'medicare');
                }elseif (is_tax('post_format', 'post-format-chat')){
                    _e('Chats', 'medicare');
                }else{
                    _e('Archives', 'medicare');
                }
            }
...
if (!function_exists('ro_theme_page_breadcrumb')) {
    function ro_theme_page_breadcrumb($delimiter) {
            ...
            if ( is_category() ) {
                ...
                echo '<span class="current">' . __('Archive by category: ', 'medicare') . single_cat_title('', false) . '</span>';

            } elseif ( is_search() ) {
                echo '<span class="current">' . __('Search results for: ', 'medicare') . get_search_query() . '</span>';
            ...
            if ( get_post_type() != 'post' ) {
                ...
                echo '<span class="current">' . __('Posts tagged: ', 'medicare') . single_tag_title('', false) . '</span>';
            } elseif ( is_author() ) {
                global $author;
                $userdata = get_userdata($author);
                echo '<span class="current">' . __('Articles posted by ', 'medicare') . $userdata->display_name . '</span>';
            } elseif ( is_404() ) {
                echo '<span class="current">' . __('Error 404', 'medicare') . '</span>';
            }

            if ( get_query_var('paged') ) {
                if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ' (';
                    echo ' '.$delimiter.' '.__('Page', 'medicare') . ' ' . get_query_var('paged');
                if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
            }
...
if ( ! function_exists( 'ro_theme_post_nav' ) ) {
	function ro_theme_post_nav() {
		...
		<nav class="container ro-blog-article-nav clearfix">
			<?php
				previous_post_link( '%link', __( '<i class="icon icon-angle-left"></i>', 'medicare' ) );
				next_post_link(     '%link',     __( 'NEXT POST<i class="icon icon-angle-right"></i>', 'medicare' ) );
			?>
		</nav>
		<?php
	}
