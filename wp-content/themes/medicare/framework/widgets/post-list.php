...
class RO_Post_List_Widget extends RO_Widget {
	public function __construct() {
		...
		$this->widget_description = __( 'Display a list of your posts on your site.', 'medicare' );
		$this->widget_id          = 'ro_post_list';
		$this->widget_name        = __( 'Post List', 'medicare' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => __( 'Post List', 'medicare' ),
				'label' => __( 'Title', 'medicare' )
			),
			'category' => array(
				'type'   => 'tb_taxonomy',
				'std'    => '',
				'label'  => __( 'Categories', 'medicare' ),
			),
...
if(function_exists('insert_widgets')) { 
	...
	function register_post_list_widget() {
		...
		insert_widgets('RO_Post_List_Widget');
