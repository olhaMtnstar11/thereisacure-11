...
class RO_Social_Widget extends WP_Widget {
    function __construct() {
        parent::__construct(
                ...
                __('Socials', 'medicare'), // Name
                array('description' => __('Social Widget', 'medicare'),) // Args
        );
    }
    ...
    function form($instance) {
		...
		?>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title:', 'medicare'); ?></label>
        ...
        for ($i = 1; $i <= 6; $i++) {
            ?>
            <p>
                <label for="<?php echo esc_url($this->get_field_id('icon_social_' . $i)); ?>"><?php _e('Social Icon:', 'medicare');
            echo ''.$i; ?></label>
            ...
            <p>
                <label for="<?php echo esc_url($this->get_field_id('link_social_' . $i)); ?>"><?php _e('Social Link:', 'medicare');
            echo ''.$i; ?></label>
        ...
        <?php } ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('el_class')); ?>"><?php _e('Extra Class:', 'medicare'); ?></label>
...
if(function_exists('insert_widgets')) { 
	...
	function register_ro_social_widget() {
		...
		insert_widgets('RO_Social_Widget');
